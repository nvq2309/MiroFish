...
Adopts project context mechanism, server-side persistent state.
"""
...
def generate_ontology():
    ...
    try:
        ...
        if not uploaded_files or all(not f.filename for f in uploaded_files):
            return jsonify(
                {"success": False, "error": "Please upload at least one document file"}
            ), 400
...
def build_graph():
    ...
    try:
        ...
        if errors:
            ...
            return jsonify(
                {"success": False, "error": "Configuration error: " + "; ".join(errors)}
            ), 500
        ...
        if not text:
            return jsonify(
                {"success": False, "error": "Extracted text content not found"}
            ), 400
        ...
        if not ontology:
            return jsonify(
                {"success": False, "error": "Ontology definition not found"}
            ), 400
        ...
        logger.info(
            f"Created graph building task: task_id={task_id}, project_id={project_id}"
        )
        ...
        def build_task():
            ...
            try:
                ...
                # Chunking
                task_manager.update_task(
                    task_id, message="Chunking text...", progress=5
                )
                ...
                # Create graph
                task_manager.update_task(
                    task_id, message="Creating Zep graph...", progress=10
                )
                ...
                # Set ontology
                task_manager.update_task(
                    task_id, message="Setting ontology definition...", progress=15
                )
                ...
                # Wait for Zep processing (query processed status for each episode)
                task_manager.update_task(
                    task_id, message="Waiting for Zep to process data...", progress=55
                )
                ...
                # Get graph data
                task_manager.update_task(
                    task_id, message="Retrieving graph data...", progress=95
                )
                ...
                build_logger.info(
                    f"[{task_id}] Graph building complete: graph_id={graph_id}, nodes={node_count}, edges={edge_count}"
                )
