...
def get_report(report_id: str):
    ...
    try:
        ...
        if not report:
            return jsonify(
                {"success": False, "error": f"Report does not exist: {report_id}"}
            ), 404
...
def chat_with_report_agent():
    ...
    Report Agent can autonomously call retrieval tools to answer questions during dialogue.
...
def get_report_sections(report_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to retrieve sections list: {str(e)}")
        return jsonify(
            {"success": False, "error": str(e), "traceback": traceback.format_exc()}
        ), 500
...
def search_graph_tool():
    ...
    try:
        ...
        if not graph_id or not query:
            return jsonify(
                {"success": False, "error": "Please provide graph_id and query"}
            ), 400
