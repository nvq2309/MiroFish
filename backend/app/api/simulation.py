...
# Adding this prefix can prevent Agent from calling tools and reply directly with text
...
def get_graph_entities(graph_id: str):
    ...
    Only returns nodes matching predefined entity types (nodes where Labels are not just Entity)
    ...
    try:
        ...
        logger.info(
            f"Getting graph entities: graph_id={graph_id}, entity_types={entity_types}, enrich={enrich}"
        )
...
def get_entity_detail(graph_id: str, entity_uuid: str):
    ...
    try:
        ...
        if not entity:
            return jsonify(
                {"success": False, "error": f"Entity does not exist: {entity_uuid}"}
            ), 404
...
def create_simulation():
    ...
    try:
        ...
        if not project_id:
            return jsonify(
                {"success": False, "error": "Please provide project_id"}
            ), 400

        project = ProjectManager.get_project(project_id)
        if not project:
            return jsonify(
                {"success": False, "error": f"Project does not exist: {project_id}"}
            ), 404
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    Note: Running scripts (run_*.py) remain in backend/scripts/ directory, no longer copied to simulation directory
    ...
    # Necessary files list (excluding scripts, which are in backend/scripts/)
    required_files = [
        "state.json",
    ...
    try:
        ...
        # Detailed log
        logger.debug(
            f"Detecting simulation preparation status: {simulation_id}, status={status}, config_generated={config_generated}"
        )
        ...
        if status in prepared_statuses and config_generated:
            ...
            if status == "preparing":
                try:
                    ...
                    logger.info(
                        f"Automatically updated simulation status: {simulation_id} preparing -> ready"
                    )
                    status = "ready"
                except Exception as e:
            ...
            logger.info(
                f"Simulation {simulation_id} detection result: Prepared (status={status}, config_generated={config_generated})"
            )
            return True, {
                "status": status,
            ...
            logger.warning(
                f"Simulation {simulation_id} detection result: Not prepared (status={status}, config_generated={config_generated})"
            )
            return False, {
...
def prepare_simulation():
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "message": "Preparation task started|Completed preparation work exists",
    ...
    try:
        ...
        if not simulation_id:
            return jsonify(
                {"success": False, "error": "Please provide simulation_id"}
            ), 400
        ...
        logger.info(
            f"Starting to process /prepare request: simulation_id={simulation_id}, force_regenerate={force_regenerate}"
        )

        # Check if already prepared (avoid duplicate generation)
        if not force_regenerate:
            logger.debug(
                f"Checking if simulation {simulation_id} is already prepared..."
            )
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            logger.debug(
                f"Check result: is_prepared={is_prepared}, prepare_info={prepare_info}"
            )
            if is_prepared:
                logger.info(
                    f"Simulation {simulation_id} already prepared, skipping duplicate generation"
                )
                return jsonify(
                    {
                        "success": True,
                        "data": {
                            ...
                            "status": "ready",
                            "message": "Completed preparation work exists, no duplicate generation required",
                ...
                logger.info(
                    f"Simulation {simulation_id} not prepared, starting preparation task"
                )
        ...
        # So frontend can get expected total Agent count immediately after calling prepare
        try:
            logger.info(
                f"Synchronously getting entity count: graph_id={state.graph_id}"
            )
            reader = ZepEntityReader()
            # Fast read entities (no edge info needed, just count)
            ...
            logger.warning(
                f"Failed to synchronously get entity count (will retry in background task): {e}"
            )
...
def get_prepare_status():
    ...
    2. Check if completed preparation work exists via simulation_id
    ...
    try:
                ...
                return jsonify(
                    {
                        ...
                        "data": {
                            ...
                            "status": "ready",
                            "progress": 100,
                            "message": "Completed preparation work exists",
                            "already_prepared": True,
                            "prepare_info": prepare_info,
                        },
                    }
        ...
        if not task_id:
            ...
            return jsonify(
                {"success": False, "error": "Please provide task_id or simulation_id"}
            ), 400
        ...
        if not task:
                    ...
                    return jsonify(
                        {
                            ...
                            "data": {
                                ...
                                "status": "ready",
                                "progress": 100,
                                "message": "Task complete (preparation work exists)",
                                "already_prepared": True,
                                "prepare_info": prepare_info,
                            },
                        }
            ...
            return jsonify(
                {"success": False, "error": f"Task does not exist: {task_id}"}
            ), 404
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    Traverse reports directory, find report matching simulation_id,
    return the latest one if multiple exist (sorted by created_at)
...
def get_simulation_history():
    ...
    Used for historical project display on home page, returns simulation list containing rich info like project name, description, etc.
    ...
    try:
        ...
        for sim in simulations:
            ...
            # Get running state (read actual rounds set by user from run_state.json)
            run_state = SimulationRunner.get_run_state(sim.simulation_id)
            if run_state:
                sim_dict["current_round"] = run_state.current_round
                sim_dict["runner_status"] = run_state.runner_status.value
                # Use total_rounds set by user, if none then use recommended rounds
...
def get_simulation_profiles(simulation_id: str):
    """
    Get Agent Profile of simulation
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Get Agent Profile of simulation in real-time (used to view progress during generation)

    Difference from /profiles interface:
    - Directly read files, not through SimulationManager
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "platform": "reddit",
                "count": 15,
                "total_expected": 93,  // Expected total count (if any)
    ...
    try:
        ...
        if file_exists:
            ...
            try:
                ...
                logger.warning(
                    f"Failed to read profiles file (may be being written): {e}"
                )
                profiles = []
...
def get_simulation_config_realtime(simulation_id: str):
    """
    Get simulation configuration in real-time (used to view progress during generation)

    Difference from /config interface:
    - Directly read files, not through SimulationManager
    - Suitable for real-time viewing during generation
    - Return additional metadata (such as file modification time, whether generating, etc.)
    - Can return partial info even if configuration generation is not complete
    ...
    try:
        ...
        if file_exists:
            ...
            try:
                ...
                logger.warning(
                    f"Failed to read config file (may be being written): {e}"
                )
                config = None
...
def download_simulation_script(script_name: str):
    """
    Download simulation running script file (general script, located in backend/scripts/)
...
def start_simulation():
    ...
    About force parameter:
        - When enabled, if simulation is running or completed, it will first stop and clean running logs
    ...
    About enable_graph_memory_update:
        ...
        - Adopts batch update mechanism to reduce API calls
    ...
    try:
        ...
        )  # Optional: whether to enable graph memory update
        ...
        if max_rounds is not None:
            try:
                ...
                return jsonify(
                    {"success": False, "error": "max_rounds must be a valid integer"}
                ), 400
            ...
            if is_prepared:
                ...
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            logger.info(
                                f"Force mode: stopping running simulation {simulation_id}"
                            )
                            try:
                                SimulationRunner.stop_simulation(simulation_id)
                            except Exception as e:
                                logger.warning(
                                    f"Warning when stopping simulation: {str(e)}"
                                )
                        else:
                            return jsonify(
                                {
                ...
                if force:
                    ...
                    if not cleanup_result.get("success"):
                        logger.warning(
                            f"Warning when cleaning logs: {cleanup_result.get('errors')}"
                        )
                    force_restarted = True

                # Process does not exist or has ended, reset status to ready
                logger.info(
                    f"Simulation {simulation_id} preparation work complete, resetting status to ready (original status: {state.status.value})"
                )
        ...
        if enable_graph_memory_update:
            ...
            if not graph_id:
                return jsonify(
                    {
                        "success": False,
                        "error": "Enabling graph memory update requires a valid graph_id, please ensure project has built a graph",
                    }
                ), 400

            logger.info(
                f"Enabling graph memory update: simulation_id={simulation_id}, graph_id={graph_id}"
            )
...
def get_run_status(simulation_id: str):
    """
    Get real-time status of simulation run (for frontend polling)
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        # recent_actions only shows content of current latest round on both platforms
...
def get_agent_stats(simulation_id: str):
    ...
    Used for frontend to display Agent activity ranking, action distribution, etc.
    """
    try:
...
def interview_agent():
    ...
    Note: This function requires the simulation environment to be in a running state (entering wait-for-command mode after completing the simulation loop)
    ...
    Request (JSON):
        {
            ...
            "platform": "twitter",             // Optional, specify platform (twitter/reddit)
                                               // When not specified: interview both platforms in dual-platform simulation
...
def interview_agents_batch():
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    // Required, interview list
                ...
                {
                    "agent_id": 1,
                    "prompt": "What is your opinion on B?"  // If platform not specified, use default value
                }
            ],
    ...
    try:
        ...
        for i, interview in enumerate(interviews):
            ...
            if item_platform and item_platform not in ("twitter", "reddit"):
                return jsonify(
                    {
                        "success": False,
                        "error": f"platform of interview list item {i + 1} can only be 'twitter' or 'reddit'",
                    }
                ), 400
        ...
        # Optimize prompt of each interview item, add prefix to avoid Agent calling tools
    ...
    Request (JSON):
        {
            ...
            "platform": "reddit",          // Optional, platform type (reddit/twitter)
                                           // If not specified, return history of both platforms
...
def close_simulation_env():
    ...
    Send close environment command to simulation, making it exit wait-for-command mode gracefully.

    Note: This is different from /stop interface, /stop will force terminate the process,
    while this interface will let simulation close environment and exit gracefully.
