...
2. Plan outline structure first, then generate by sections
3. Each section uses ReACT multi-turn thinking and reflection mode
...
class ReportLogger:
    ...
    def _get_elapsed_time(self) -> float:
        """Get the elapsed time from start (seconds)"""
...
This is not an LLM simulation, but a call to a real interview interface to obtain original answers from simulation Agents.
...
4. Call the /api/simulation/interview/batch interface for real interviews on dual platforms
...
PLAN_SYSTEM_PROMPT = """\
You are an expert in writing "Future Prediction Reports," possessing a "God's eye view" of the simulated world — you can insight into the behavior, speech, and interaction of every Agent in the simulation.

[Core Concept]
We have constructed a simulated world and injected specific "simulation requirements" as variables. The evolution results of the simulated world are predictions of what might happen in the future. What you are observing is not "experimental data," but a "rehearsal of the future."
...
2. [Must quote original words and deeds of Agents]
   ...
   - Use quote format in the report to display these predictions, for example:
     > "A certain group of people will state: original content..."
...
3. [Language Consistency - All output must be in English]
   ...
   - The report must always be written entirely in English regardless of input language
   - When you quote content returned by tools in other languages, translate it into fluent English before writing it into the report
...
4. [Faithfully present prediction results]
   ...
   - If information in some aspect is insufficient, state it truthfully
...
Weibo, as the first scene of public opinion, undertook the core function of initial information release:

> "Weibo contributed 68% of the initial volume..."

**Emotion Amplification Stage**

The Douyin platform further amplified the influence of the event:
...
- interview_agents: Interview simulation Agents to obtain first-person views and real reactions of different roles

═══════════════════════════════════════════════════════════════
[Workflow]
═══════════════════════════════════════════════════════════════

In each reply, you can only do one of the following two things (cannot do both simultaneously):
...
2. Use a large number of original quotes to show simulation effects
3. Use Markdown format (but forbidden to use headings):
...
7. [Emphasize Again] Do not add any headings! Use **bold** instead of sub-section headings"""
...
2. Then Action to call tools to obtain simulation data
3. Output Final Answer (pure body, no headings) after collecting enough information"""

# ── ReACT Inner Loop Message Templates ──
...
REACT_UNUSED_TOOLS_HINT = "\n💡 You haven't used: {unused_list} yet, it's recommended to try different tools to get multi-angle information"
...
2. Answer questions directly, avoiding lengthy thought discourse
...
class ReportAgent:
    ...
    2. Generation stage: Generate content section by section, each section can call tools multiple times to obtain information
    ...
    # Legal tool name set, used for validation during bare JSON fallback parsing
    VALID_TOOL_NAMES = {
        "insight_forge",
        "panorama_search",
        "quick_search",
    ...
    def _generate_section_react(
        ...
        # Build user prompt - each completed section passed in with a maximum of 4000 characters
        if previous_sections:
            previous_parts = []
            for sec in previous_sections:
                # Each section maximum 4000 characters
        ...
        for iteration in range(max_iterations):
            ...
            if response is None:
                ...
                # If there are still iterations, add message and retry
                if iteration < max_iterations - 1:
                    messages.append(
                        {"role": "assistant", "content": "(Response is empty)"}
                    )
                    messages.append(
                        {
                            "role": "user",
                    ...
                    continue
                # Last iteration also returns None, break loop and enter forced completion
                break
            ...
            if has_tool_calls and has_final_answer:
                ...
                logger.warning(
                    ...
                    f"LLM simultaneously output tool call and Final Answer ({conflict_retries} conflict)"
                )
            ...
            if has_final_answer:
                ...
                if tool_calls_count < min_tool_calls:
                    ...
                    unused_hint = (
                        f"(These tools have not been used yet, it is recommended to use them: {', '.join(unused_tools)})"
                        if unused_tools
                        else ""
                    )
                    messages.append(
                ...
                logger.info(
                    f"Section {section.title} generation complete (Tool calls: {tool_calls_count} times)"
                )
            ...
            if has_tool_calls:
                # Tool quota exhausted -> Clearly inform, ask to output Final Answer
                ...
                if len(tool_calls) > 1:
                    logger.info(
                        f"LLM attempted to call {len(tool_calls)} tools, only executing the first one: {call['name']}"
                    )
            ...
            # Directly take this content as the final answer, no more idling
            logger.info(
                f"Section {section.title} No 'Final Answer:' prefix detected, directly adopting LLM output as final content (Tool calls: {tool_calls_count} times)"
            )
        ...
        logger.warning(
            f"Section {section.title} Maximum iterations reached, forcing generation"
        )
        ...
        if response is None:
            logger.error(
                f"Section {section.title} LLM returned None during forced completion, using default error hint"
            )
    ...
    def generate_report(
        ...
        """
        Generate full report (real-time output by section)
        ...
        # If report_id is not passed, automatically generate it
        if not report_id:
        ...
        try:
            ...
            ReportManager.update_progress(
                ...
                "planning",
                15,
                f"Outline planning complete, total {len(outline.sections)} sections",
                completed_sections=[],
            )
            ReportManager.save_report(report)
    ...
    def chat(
        ...
        In the dialogue, the Agent can autonomously call retrieval tools to answer questions
...
class ReportManager:
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        Args:
            report_id: Report ID
            from_line: From which line to start reading (for incremental acquisition, 0 means from the beginning)
    ...
    def get_agent_log_stream(cls, report_id: str) -> List[Dict[str, Any]]:
        """
        Get full Agent log (for getting all at once)
    ...
    def save_section(
        ...
        Called immediately after each section generation is complete, implementing output by section
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        while cleaned_lines and cleaned_lines[0].strip() in ["---", "***", "___"]:
            cleaned_lines.pop(0)
            # Also remove empty lines after the separator line
            while cleaned_lines and cleaned_lines[0].strip() == "":
                cleaned_lines.pop(0)
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        3. Clean up redundant empty lines and separator lines
        ...
        # Collect all section titles in the outline
        section_titles = set()
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Check if it is a duplicate heading (heading with the same content appearing within 5 consecutive lines)
                is_duplicate = False
                ...
                if is_duplicate:
                    # Discard duplicate heading and subsequent empty lines
                    i += 1
                ...
                if level == 1:
                    if title == outline.title:
                        ...
                        # Other level 1 headings turned to bold
                        processed_lines.append(f"**{title}**")
                    ...
                    if title in section_titles or title == outline.title:
                        ...
                        # Non-section level 2 headings turned to bold
                        processed_lines.append(f"**{title}**")
                        processed_lines.append("")
                        prev_was_heading = False
                else:
                ...
                # Skip the separator line immediately following the heading
                i += 1
                continue

            elif stripped == "" and prev_was_heading:
                # Only keep one empty line after the heading
