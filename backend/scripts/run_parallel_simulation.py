...
- Does not close environment immediately after completing simulation, enters wait-for-command mode
...
# ============================================================
# Solve Windows encoding issue: set UTF-8 encoding before all imports
...
if sys.platform == "win32":
    ...
    # Reconfigure standard output stream to UTF-8 (solve console Chinese garbled characters)
    ...
    # Note: This needs to be set when Python starts, setting at runtime may not take effect
    ...
    def _utf8_open(
        ...
        This can fix the issue where third-party libraries (such as OASIS) read files without specifying encoding
        """
        # Only set default encoding for text mode (non-binary) and when encoding is not specified
...
# Scripts are fixed in backend/scripts/ directory
...
# Load .env file in project root directory (contains LLM_API_KEY etc. configurations)
...
def disable_oasis_logging():
    """
    Disable detailed log output of OASIS library
    OASIS logs are too redundant (recording observations and actions of each agent), we use our own action_logger
    """
    # Disable all loggers of OASIS
    oasis_loggers = [
        "social.agent",
        "social.twitter",
        "social.rec",
...
def init_logging_for_simulation(simulation_dir: str):
    """
    Initialize log configuration for simulation

    Args:
        simulation_dir: Simulation directory path
    """
    # Disable detailed logs of OASIS
...
class ParallelIPCHandler:
    ...
    Manages environments of both platforms, handles Interview commands
    """
    ...
    def _get_env_and_graph(self, platform: str):
        """
        Get environment and agent_graph of specified platform
    ...
    async def handle_interview(
        ...
        Args:
            ...
            platform: Specified platform (optional)
                ...
                - None/Not specified: Interview both platforms simultaneously, return integrated result
        ...
        if success_count > 0:
            ...
            print(
                f"  Interview complete: agent_id={agent_id}, successful platforms={success_count}/{len(platforms_to_interview)}"
            )
            return True
        else:
            errors = [
    ...
    async def process_commands(self) -> bool:
        ...
        if command_type == CommandType.INTERVIEW:
            ...
            self.send_response(
                command_id, "completed", result={"message": "Environment is closing"}
            )
            return False
...
# Core action types to keep (others are filtered out as they have low analysis value)
...
def fetch_new_actions_from_db(
    ...
    """
    Get new action records from database and supplement with full context information

    Args:
        db_path: Database file path
        last_rowid: Maximum rowid value from last read (use rowid instead of created_at because created_at formats differ across platforms)
    ...
    try:
        ...
        # Use rowid to track processed records (rowid is a built-in auto-increment field in SQLite)
...
def _enrich_action_context(
    ...
    try:
        ...
        # Repost: supplement original post content and author
            ...
            if new_post_id:
                # original_post_id of reposted post points to original post
                cursor.execute(
                    """
        ...
        # Quote post: supplement original post content, author and quote comment
            ...
            # Get comment content of quoted post (quote_content)
            if new_post_id:
                cursor.execute(
                    """
        ...
        # Create comment: supplement info of the post being commented on
...
def _get_post_info(
    ...
    try:
        ...
        if row:
            ...
            # Prioritize name in agent_names
            author_name = ""
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        raise ValueError(
            "Missing API Key configuration, please set LLM_API_KEY in project root .env file"
        )
...
def get_active_agents_for_round(
    ...
    """Decide which Agents to activate in this round based on time and configuration"""
...
async def run_twitter_simulation(
    ...
    total_actions = 0
    last_rowid = 0  # Track last processed row number in database (use rowid to avoid created_at format difference)
    ...
    for round_num in range(total_rounds):
        ...
        if _shutdown_event and _shutdown_event.is_set():
            if main_logger:
                main_logger.info(
                    f"Received exit signal, stopping simulation at round {round_num + 1}"
                )
            break
...
async def run_reddit_simulation(
    ...
    # Reddit uses boost LLM configuration (if any, otherwise falls back to general configuration)
...
async def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Close environment immediately after simulation completion, do not enter wait-for-command mode",
    )
    parser.add_argument(
        "--twitter-only",
        action="store_true",
    ...
    log_manager.info("=" * 60)
    log_manager.info("OASIS Dual-Platform Parallel Simulation Starting")
    log_manager.info(f"Configuration file: {args.config}")
    log_manager.info(
        f"Wait-for-command mode: {'Enabled' if not args.no_wait else 'Disabled'}"
    )
    log_manager.info("=" * 60)
    ...
    if run_twitter:
        tasks.append(
            run_twitter_simulation(
                config,
                simulation_dir,
                action_logger=log_manager.get_twitter_logger(),
    ...
    twitter_sim = platform_results[0] if run_twitter else PlatformSimulation()
    reddit_sim = (
        platform_results[1]
        if (run_twitter and run_reddit)
        else (platform_results[0] if run_reddit else PlatformSimulation())
    )

    ipc_handler = ParallelIPCHandler(
        simulation_dir,
        twitter_env=twitter_sim.env,
        twitter_agent_graph=twitter_sim.agent_graph,
        reddit_env=reddit_sim.env,
        reddit_agent_graph=reddit_sim.agent_graph,
    )

    # Whether to enter wait-for-command mode
    if not args.no_wait:
        log_manager.info("\n" + "=" * 60)
        log_manager.info("Entering wait-for-command mode - Environment remains running")
    ...
    close_tasks = []
    if twitter_sim.env:
        close_tasks.append(twitter_sim.env.close())
    if reddit_sim.env:
        close_tasks.append(reddit_sim.env.close())

    if close_tasks:
        await asyncio.gather(*close_tasks)

    log_manager.info("Environment closed")
    log_manager.info("=" * 60)


def setup_signal_handlers():
    """
    Set signal handlers to ensure correct exit when receiving SIGTERM/SIGINT
    Allows program to clean up resources normally (close database, environment, etc.)
    """
