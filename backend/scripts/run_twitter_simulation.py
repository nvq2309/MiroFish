...
class TwitterSimulationRunner:
    ...
    def _create_model(self):
        ...
        Uniformly use configurations in project root .env file (highest priority):
        - LLM_API_KEY: API key
        ...
        print(
            f"LLM configuration: model={llm_model}, base_url={llm_base_url[:40] if llm_base_url else 'Default'}..."
        )
    ...
    async def run(self, max_rounds: int = None):
        ...
        print(
            f"Wait-for-command mode: {'Enabled' if self.wait_for_commands else 'Disabled'}"
        )
        print("=" * 60)
        ...
        if initial_posts:
            ...
            for post in initial_posts:
                ...
                try:
                    ...
                    print(
                        f"  Warning: Unable to create initial post for Agent {agent_id}: {e}"
                    )
        ...
        for round_num in range(total_rounds):
            ...
            # Get activated Agents in this round
        ...
        if self.wait_for_commands:
            print("\n" + "=" * 60)
            print("Entering wait-for-command mode - Environment remains running")
...
async def main():
    ...
    # Initialize log configuration (use fixed filename, clean old logs)
