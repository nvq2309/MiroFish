    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Automatically call tools to extract entities and relationships from the knowledge graph based on context, initialize simulated individuals, and endow them with unique behaviors and memories based on real-world seeds
          </p>
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            ...
            <div class="stat-card">
              ...
              <span class="stat-label">Topics Associated with Seeds</span>
            </div>
          </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            The LLM intelligently sets parameters such as world time flow, recommendation algorithms, active time periods for each individual, speaking frequency, and event triggers based on simulation requirements and real-world seeds
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Automatically generate initial activation events and hot topics based on the narrative direction to guide the initial state of the simulated world
          </p>
              ...
              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  ...
                  <div class="auto-content">
                    ...
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">For the first run, it is strongly recommended to switch to 'Custom Mode' to reduce simulation rounds for a quick preview and to lower the risk of errors ➝</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div v-if="selectedProfile" class="profile-modal-overlay" @click.self="selectedProfile = null">
        ...
        <div class="modal-body">
          ...
          <div class="modal-info-grid">
            ...
            <div class="info-item">
              <span class="info-label">Event Apparent Gender</span>
            ...
            <div class="info-item">
              <span class="info-label">Event Apparent MBTI</span>
          ...
          <div class="modal-section" v-if="selectedProfile.interested_topics?.length">
            <span class="section-label">Topics Associated with Seeds</span>
...
// Log deduplication: record the last logged key message
...
watch(currentStage, (newStage) => {
  if (newStage === 'Generate Agent Profiles' || newStage === 'generating_profiles' || newStage === '生成Agent人设') {
    phase.value = 1
  } else if (newStage === 'Generate Simulation Config' || newStage === 'generating_config' || newStage === '生成模拟配置') {
    phase.value = 2
    // Enter config generation phase, start polling config
    if (!configTimer) {
      addLog('Start generating dual-platform simulation config...')
      startConfigPolling()
    }
  } else if (newStage === 'Prepare Simulation Scripts' || newStage === 'copying_scripts' || newStage === '准备模拟脚本') {
...
const autoGeneratedRounds = computed(() => {
  if (!simulationConfig.value?.time_config) {
    return null // Return null when config is not generated
  }
...
// Calculate total associated topics for all profiles
...
const startPrepareSimulation = async () => {
  ...
  // Mark first step completed, start second step
  phase.value = 1
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Immediately set expected total agents (get from prepare API return value)
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.config_generated && data.config) {
        ...
        if (data.summary) {
          addLog(`  ├─ Agents count: ${data.summary.total_agents}`)
        ...
        if (data.config.time_config) {
          ...
          addLog(`Time config: ${tc.minutes_per_round} mins/round, total ${Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round)} rounds`)
        }
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        // Show detailed config summary
        if (res.data.summary) {
          addLog(`  ├─ Agents count: ${res.data.summary.total_agents}`)
        ...
        addLog('Config generating, start polling to wait...')
        startConfigPolling()
      }
    }
  } catch (err) {
