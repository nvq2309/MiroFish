...
const handleNextStep = (params = {}) => {
  ...
  // If custom rounds, pass via query
  if (params.maxRounds) {
 ...
 * When user goes back from Step 3 to Step 2, default to exit simulation
 */
const checkAndStopRunningSimulation = async () => {
  if (!currentSimulationId.value) return
  
  try {
    // Check if simulation env is alive
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      // Env not running, but process might be, check simulation status
    ...
    // Checking status failure doesn't affect flow
